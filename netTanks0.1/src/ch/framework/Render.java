...
public class Render {
    ...
    public Render(Framework framework) {
        this.framework = framework;

        ground = new Canvas(framework.getWidth(), framework.getHeight());
        groundGc = ground.getGraphicsContext2D();
        framework.getChildren().add(ground);
        groundGc.setFill(Color.BEIGE);
        groundGc.fillRect(0, 0, framework.getWidth(), framework.getHeight());


        primary = new Canvas(framework.getWidth(), framework.getHeight());
        primaryGc = primary.getGraphicsContext2D();
        framework.getChildren().add(primary);
    }

    public void render(Handler handler) {
        //Clear Canvas (Prevents "smearing" effect)
        primaryGc.clearRect(0, 0, framework.getWidth(), framework.getHeight());
